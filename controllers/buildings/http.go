...
import (
	...
	"capstone/controllers/buildings/request"
...
func (buildingController *BuildingController) GetAll(c echo.Context) error {
	...
	return controllers.NewSuccesResponse(c, response.FromDomainBuildingAll(Building))
...
func (buildingController *BuildingController) GetByID(c echo.Context) error{
	...
	data, err := buildingController.buildingUseCase.GetByID(req, uint(Convint))
	if err != nil {
...
func (buildingController *BuildingController) GetByComplexID(c echo.Context) error{
	...
	return controllers.NewSuccesResponse(c, response.FromDomainBuildingAll(data))
}

func (buildingController *BuildingController) Update (c echo.Context) error{
	id := c.Param("id")
	...
	ctx := c.Request().Context()
	data, err := buildingController.buildingUseCase.Update(ctx, convID,*req.ToDomain())
...
func (buildingController *BuildingController) Delete(c echo.Context) error {
	...
	ctx := c.Request().Context()
	err = buildingController.buildingUseCase.Delete(ctx, convID)
